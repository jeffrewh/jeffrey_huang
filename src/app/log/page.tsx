  ...
  return (
    <div className="mx-auto max-w-screen-md py-12 px-6">
      ...
      <p className="text-slate-400 max-w-lg mb-8">
        A chronological log of significant updates to this site. Commits with "|
        omit" in their message are excluded.
      </p>

      {loading && <p className="text-slate-500">Loading commit history...</p>}
      {error && (
        <p className="text-red-400">Error loading commit history: {error}</p>
      )}

      {!loading && commits.length === 0 && !error && (
        <p className="text-slate-500">
          No commits found yet. All commits will show by default unless tagged
          with "| omit".
        </p>
      )}
      ...
      <ul className="space-y-6">
        {commits.map((commit) => (
          <li
            key={commit.hash}
            className="group relative grid grid-cols-1 md:grid-cols-4 gap-4 pb-1 transition-all"
          >
            ...
            <time
              ...
              className="z-10 text-xs font-semibold uppercase tracking-wide text-slate-500 md:col-span-1"
            >
            ...
            <div className="z-10 md:col-span-3">
              ...
              <p className="mt-1 text-sm text-slate-500">
                By {commit.author} &bull;{" "}
                <code className="text-xs">{commit.hash.substring(0, 7)}</code>
              </p>
            </div>
          </li>
        ))}
